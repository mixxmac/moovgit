...
mylists.addToCart.init = function() {
	jQuery(document).on("click", ".add-cart", function(event) {
		...
		items.each(function(i, e) {
			...
			jQuery.post(mylists.vars.urls.addToCart, form.serialize(), function(d, s, x) {
...
mylists.addItemsList = {
	...
	show: function(itemId, hashCode, listName) {
		var data = {
			itemId: itemId,
			hashCode: hashCode,
			listName: listName,
		...
		if (jQuery("#added-to-list").length < 1) {
			...
			if (jQuery("#added-to-list #items-added").length > 0) {
				...
				if (itemsAdded.length > 1) {
					jQuery("#items-added").append("<li><h5>" + itemsAdded.length + " Items</h5></li>");
				} else {
					jQuery("#items-added").append("<li><h5>" + jQuery(itemsAdded).parent().siblings(".title-wrap").children(".title").children().html() + "</h5></li>");
				}
			}
		}
	...
	addHandler: function(listName, listCount, listHashCode, listId) {
		var itemCount = 0;
		var container = jQuery(mylists.vars.currentButton.closest(".productDescriptionBlock"));
	...
	pageReload: function() {
		...
		if (mylists.vars.currentButton.length > 0) {
			...
			jQuery("html,body").animate({scrollTop: mylists.vars.currentButton.offset().top - 150}, "slow");
		}
	}
};
...
mylists.addItemsList.init = function() {
	...
	jQuery("body").click(function(e) {
		if (jQuery(e.target).attr("id") !== "list-dropdown"
				&& !jQuery(e.target).hasClass("add-items")) {
			jQuery("#list-dropdown").remove();
		}
	});
...
mylists.widget.init = function() {
	if (jQuery("#mylists-widget").length > 0) {
		...
		mylists.ajax(mylists.vars.urls.getGNCLists, apiData, function(data, textStatus, jqXHR) {
			var items = [data[""].title, data[""].wishListItems, {loggedIn: loggedIn}];
