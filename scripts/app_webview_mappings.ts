...
match($status) {
  ...
  with(/200/) {
    log("--> STATUS: 200")
    match($path) {
      with(/pagetype=shop/i) {
        # http://www.gnc.com/home/index.jsp
        log("--> Importing app_pages/webviews/shop.ts in app_webview_mappings.ts")
        @import "app_pages/webviews/shop.ts"
      }
      
      else() {
        log("--> No page match in app_webview_mappings.ts")
      }
    }
  }
