...
getURI() {
  PATH1="./$DIRTO/${2:2}"
  URI="${1:5}"
  [ "${1:0:5}" == '#URI=' ] && echo -e "\n$NEWBAR"  && echo "Backing up $2" && wget "$URI" -o dumpfile -O "$PATH1.html" && ./wkhtmltopdf --disable-javascript -q -l ${URI/www/mstage} "$PATH1.pdf" && saveJS "$PATH1.html" $(grep "Resolving" dumpfile -B 1 | tail -2 | head -1 | grep -o "http.*")
}
